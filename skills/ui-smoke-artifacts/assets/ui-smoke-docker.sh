...
for var in BASE_URL \
  ...
  UI_LAYOUT_PATH UI_LAYOUT_RUN_ID UI_LAYOUT_OUTPUT UI_LAYOUT_SPACING UI_LAYOUT_OVERLAP_TOLERANCE UI_LAYOUT_TABLET UI_LAYOUT_IGNORE_DEV_INDICATOR \
  UI_SNAPSHOT_PATH UI_SNAPSHOT_UPDATE UI_SNAPSHOT_THRESHOLD UI_SNAPSHOT_SAVE_ACTUAL \
  UI_MOBILE_VIEWPORT \
  UI_DOCKER_CMD; do
  if [[ -n "${!var:-}" ]]; then
    ENV_FLAGS+=(-e "$var=${!var}")
